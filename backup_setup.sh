...
if [ -n "`echo ${desc_bk} | grep -w 8`" ]; then
  if [ ! -e "/usr/local/bin/dbxcli" ]; then
    if [ "${armplatform}" == 'y' ]; then
      wget -qc https://raw.githubusercontent.com/sshadowrocket/ois/main/src/dbxcli-linux-arm -O /usr/local/bin/dbxcli
    else
      wget -qc https://raw.githubusercontent.com/sshadowrocket/ois/main/src/dbxcli-linux-amd64 -O /usr/local/bin/dbxcli
    fi
