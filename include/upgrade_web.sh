...
Upgrade_Tomcat() {
  ...
  while :; do echo
    ...
    if [ "`echo ${NEW_tomcat_ver} | awk -F. '{print $1}'`" == "${Tomcat_flag}" ]; then
      ...
      src_url=https://raw.githubusercontent.com/sshadowrocket/ois/main/apache/tomcat/v${NEW_tomcat_ver}/apache-tomcat-${NEW_tomcat_ver}.tar.gz && Download_src
      ...
      if [ -e "${tomcat_install_dir}/lib/catalina-jmx-remote.jar" ]; then
        src_url=https://raw.githubusercontent.com/sshadowrocket/ois/main/apache/tomcat/v${NEW_tomcat_ver}/catalina-jmx-remote.jar && Download_src
