...
Install_Python() {
  if [ -e "${python_install_dir}/bin/python" ]; then
    ...
    if [ ! -e "${python_install_dir}/bin/python" -a ! -e "${python_install_dir}/bin/python3" ] ;then
      src_url=https://raw.githubusercontent.com/sshadowrocket/ois/main/src/Python-${python_ver}.tgz && Download_src
    ...
    if [ ! -e "${python_install_dir}/bin/pip" ]; then
      src_url=https://raw.githubusercontent.com/sshadowrocket/ois/main/src/setuptools-${setuptools_ver}.zip && Download_src
      src_url=https://raw.githubusercontent.com/sshadowrocket/ois/main/src/pip-${pip_ver}.tar.gz && Download_src
