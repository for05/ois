...
Upgrade_OneinStack() {
  ...
  Latest_OneinStack_MD5=$(curl --connect-timeout 3 -m 5 -s https://raw.githubusercontent.com/sshadowrocket/ois/main/md5sum.txt | grep oneinstack.tar.gz | awk '{print $1}')
  ...
  if [ "${oneinstack_md5}" != "${Latest_OneinStack_MD5}" ]; then
    ...
    wget --no-check-certificate -qc https://raw.githubusercontent.com/sshadowrocket/ois/main/oneinstack.tar.gz -O /tmp/oneinstack.tar.gz
    ...
    if [ -e "change_jdk_version.sh" ]; then
      ...
      wget -qc https://raw.githubusercontent.com/sshadowrocket/ois/main/scripts/change_jdk_version.sh
